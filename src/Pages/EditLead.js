...
import {
    ...
    LeadHistoryStatus, CheckEmailMobileNumberExists
} from "../apicaller";
import {
    Grid, Alert, Snackbar
} from "@mui/material";
...
const EditLead = () => {
    ...
    const SetHistoryDetails = async (leadid, userid, token) => {
    ...
    useEffect(() => {
        if (!rights.updatelead[typeuser]?.includes(department) && typeuser != "admin") {
        ...
        (async function effect() {
            if (leadid) {
                ...
                const leadddetail = await LeadData(leadid, userid, token_).then(async (response) => {
                    if (response.data._code === 200) {
                        await setLeadData(response.data.data)
                    }
                    else {
                        navigate("/app/leads")
                    }
                })
                await Users(false, userid, token_).then((response) => {
                    dispatchstate({ "users": response.data.data })
                })
                await GetKeywords().then((response) => {
                    if (response.data._code === 200) {
                        setKeywords(response.data.data)
                    }
                })
                await GetKeywordsOfLead(leadid).then((response) => {
                ...
                await SetHistoryDetails(leadid, userid, token_)
                return true
            }
            else {
                                            ...
                                            CheckEmailMobileNumberExists({ "email": lead.email, "mobile": lead.mobile, "leadid": leadid, "gstin": lead.gstin }).then((response) => {
                                                ...
                                                if (check_exists_data._code == 200 && !check_exists_data.exists) {
                                                    ModifyLead(lead).then((response) => {
                                                        let data = response.data
                                                        if (data._code === 200) {
                                                            (async () => await SetHistoryDetails(leadid, userid, token_))();
                                                            ...
                                                            historiesdetails.map((response) => {
                                                                return (
                                                                    <tr>
                                                                        ...
                                                                        <td> {response["update_date"]} / {response["update_time"]} </td>
                                                                    </tr>
                                                                )
                                                            })
                                                        }
