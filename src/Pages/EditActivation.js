...
import { ActivationDetail, ActivationHistoriesData, UpdateClientStatus, AddTaskClient, ChangeActivation, TasksData, Users } from "../apicaller";
import statescities from "../States_Cities.json"
import { ImportPackageActivation, rights } from "../ImportVariables";
...
export const EditActivation = () => {
    ...
    const StatusSubmit = (event) => {
        ...
        if (getdetail.client_talk_desc != formdata.description || getdetail.status != formdata.status) {
            UpdateClientStatus(formdata).then((response) => {
            ...
            <div className="content-wrapper">
                <div className="page-header">
                    <h3 className="page-title">
                        ...
                        <Link to={"/app/clients"} style={{ color: "black", textDecoration: 'none' }}>Clients</Link>/{getdetail.contactpers && (getdetail.contactpers + "(" + getdetail.clientid + ")")}
                    </h3>
                </div>
                <Grid className="row">
                                    ...
                                    <div className="form-group row" style={{ marginBottom: 2, marginLeft: 2 }}>
                                        ...
                                        <div className="col-sm-4">
                                            <select className="form-control" id="exampleInputTechnicalPerson" onChange={({ target }) => ActivationDetailAdd('tech_id', target.value)}>
                                                {mainstate.users.map((response) => {
                                                    if (response.department == "Techanical") {
                                                        return (
                                            ...
                                            <button type="button" className="btn btn-gradient-primary me-2" onClick={() => {
                                                ChangeActivation(getdetail).then((response) => {
                                                    ...
                                                    if (GetDetail._code == 200) {
                                                        ...
                                                        dispatchstate({ 'alertdata': SetAlertData(true, GetDetail.message, "error") })
                                                    }
                                                })
                                            }}>Submit</button>
                                        </div>
                ...
                {(rights.changeclients[typeuser]?.includes(department) || typeuser == "admin") &&
                ...
                {(rights.taskclient.typeuser.includes(typeuser) || rights.taskclient.department.includes(department)) || (typeuser == "admin") && (
                    <>
